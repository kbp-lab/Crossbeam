...
# Function to plot FWHM resolutions of the three arrangements (run this cell)

def plot_FWHM_dB(single_trxd, antiparallel_trxd, orthogonal_trxd, orthogonal_pressure_trimmed,
    col, x_ap, figure_path=None):
    ...
    plt.plot(x_ap, 20 * 
          np.log10(antiparallel_trxd.max_pressure[300, :]/ 
                    np.max(antiparallel_trxd.max_pressure)), 
...
# plot the single transducer fields 
def plot_antiparallel_trxd_fields(max_pressure, dB_intensity, vx, vy, x, y, fr, dx, extent, figure_path):
    ...
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.set_title('(b) Axial Beam Profile')
    ax2.plot(x, dB_intensity[300, :], color='purple')
    ax2.set_xticks([])
    ax2.set_ylabel(r'$\mathrm{dB}$', fontsize=16)
    ax2.tick_params(axis='y', which='major', labelsize=14)
    plt.axhline(y=-3, color='k', ls=':', lw=1)
    ...
    ax3 = fig.add_subplot(gs[1, 1])
    ax3.set_title('(c) Lateral Beam Profile')
    ax3.plot(y, dB_intensity[:, 300], color='tab:orange')
    ax3.set_ylabel(r'$\mathrm{dB}$', fontsize=16)
    ax3.tick_params(axis='both', which='major', labelsize=14)
    plt.axhline(y=-3, color='k', ls=':', lw=1)
    ax3.set_xlabel('$\mathrm{mm}$', fontsize=16)
...
# Function to generate orthogonal transducers animations
...
# Function to animate diagonal and antidiagonal pressures
...
# function to plot ARF quiver plot for antiparalle transducers 
def plot_antiparallel_trxd_ARF_quiver(p_max, Fx, Fy, file_name, scale, col, 
                                dx, Nx, dy, Ny, X, Y, qres, figure_path):
    plt.figure(figsize=(12, 8))

    plt.title(r'$P_{max}$ + ARF', fontsize=24)
    plt.imshow(1e-6 * p_max[:, col:-col], cmap='jet', 
            extent=(col * dx * 1e3, (Nx - col) * dx * 1e3, Ny * dy * 1e3, 0))
    ...
    plt.ylabel(r'mm', fontsize=22)
    plt.xlabel(r'mm', fontsize=22)
    plt.xticks(fontsize=14)
    plt.yticks(np.arange(20, 41, 5), fontsize=14)
    plt.xlim(25, 35)
    plt.ylim(32, 28)
